...
func main() {	
	...
	resp,error := gunzip.DownloadUrl(srcUrl)	
	defer gunzip.CloseResponse(resp)	
	checkError(error)
	
	reader, error := gunzip.UnGzip(&resp.Body)
	checkError(error)
	
	
	tarreader, error := gunzip.UnTar(reader)
	checkError(error)
	
	error = gunzip.SaveTarToDisk(tarreader, destinationDir);
	checkError(error)
